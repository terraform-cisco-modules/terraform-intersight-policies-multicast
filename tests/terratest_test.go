...
func TestFull(t *testing.T) {
	...
	instanceName := fmt.Sprintf("test-policies-multicast-%s", uniqueId)
...
{
	"Name":        "{{ .name }}",
	"Description": "{{ .name }} Multicast Policy.",

	"QuerierIpAddress": "198.18.2.5",
	"QuerierIpAddressPeer": "198.18.2.6",
...
`
	...
	iassert.AssertMOComply(t, fmt.Sprintf("/api/v1/fabric/MulticastPolicies/%s", moid), expectedJSONTemplate, vars)
}
